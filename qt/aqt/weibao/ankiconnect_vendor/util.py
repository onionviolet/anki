...
DEFAULT_CONFIG = {
    ...
    'webBindPort': int(os.getenv('ANKICONNECT_BIND_PORT', '8765')),
...
def configure_fork(config):
    global _fork_config
    _fork_config = config

def setting(key):
    if _fork_config is not None:
        return _fork_config.get(key, DEFAULT_CONFIG[key])
    try:
